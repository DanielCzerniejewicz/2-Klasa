...
namespace _10_3_konstruktory
{
  internal class Program
  {
    ...
    private static void RemoveAnimal(List<Animal> animals)
    {
      ...
      {
        Console.WriteLine("1. Usun zwierzaka o numerze");
        Console.WriteLine("2. Usuń wszystkie zwierzaki z listy");
        string choice = Console.ReadLine();
        switch(choice)
        {
          case "1":
            Console.WriteLine("Podaj numer zwierzaka do usuniecia");
            int index = int.Parse(Console.ReadLine()) - 1;
            animals.RemoveAt(index);
            break;
          case "2":
            Console.WriteLine("Usunięto wszystkie zwierzaki!");
            animals.Clear();
            break;
          default:
            Console.WriteLine("Podano zły numer wybierz jeszcze raz");
            return;
        }
      }
      ShowMainMenu(animals);
    ...
    private static void ShowAnimalDetails(List<Animal> animals)
    {
      ...
      if (animals.Count() == 0)
      {
        Console.WriteLine("Brak zwierzakow na liscie");
      }
      else
      {
        ...
        {
          Console.WriteLine("Niepoprawny numer. Spróbuj ponownie dzbanie.");
        }
        ShowMainMenu(animals);
      }
    }
    ...
    private static void ShowAnimalList(List<Animal> animals)
    {
      ...
      if (animals.Count == 0)
      {
        Console.WriteLine("Pusta lista zwierzaków");
      }
      else
      {
        for (int i = 0; i < animals.Count; i++)
        {
          Console.Write($"{i+1} {animals[i].Name}");
        }
      }
      ShowMainMenu(animals);
    }
